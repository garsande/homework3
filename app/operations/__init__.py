# This file is now called "Operations.py" which is based on OOPs principles
# and it contains a class `Operations` with four static methods: addition, subtraction, multiplication, and division.
# These methods are encapsulated within the `Operations` class, providing a structured way to perform basic math operations.
# When we need to add, subtract, multiply, or divide numbers, we can call these static methods directly through the class wihout instantiating.
...
class Operations:
    ...
    def division(a: float, b: float) -> float:
        ...
        Since dividing by zero doesn't work. we need to check before we divide.
